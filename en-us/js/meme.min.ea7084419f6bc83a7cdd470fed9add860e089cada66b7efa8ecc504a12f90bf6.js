wrapTable();function wrapTable(){const tables=document.querySelectorAll('div.post-body table');for(let i=0;i<tables.length;i++){const parent=tables[i].parentNode;const wrapper=document.createElement('div');wrapper.className='table-container';parent.replaceChild(wrapper,tables[i]);wrapper.appendChild(tables[i]);}};backToTop();function backToTop(){var backToTop=document.getElementById('back-to-top');if(backToTop!==null){window.addEventListener('scroll',function(){window.scrollY>100?backToTop.classList.add('show'):backToTop.classList.remove('show');});}};if(userPrefers==='dark'){changeMode('dark');}else if(userPrefers==='light'){changeMode('light');}else if(darkModeMediaQuery.matches){changeMode('dark');}else if(lightModeMediaQuery.matches){changeMode('light');}
darkModeMediaQuery.addListener((e)=>{const darkModeOn=e.matches;if(darkModeOn){changeModeMeta('dark');changeMode('dark');setMode('dark');}});lightModeMediaQuery.addListener((e)=>{const lightModeOn=e.matches;if(lightModeOn){changeModeMeta('light');changeMode('light');setMode('light');}});function modeSwitcher(){const currentMode=document.documentElement.getAttribute('data-theme');if(currentMode==='dark'){changeModeMeta('light');changeMode('light');setMode('light');}else{changeModeMeta('dark');changeMode('dark');setMode('dark');}}
window.addEventListener('storage',function(event){if(event.newValue==='dark'){changeModeMeta('dark');changeMode('dark');}else{changeModeMeta('light');changeMode('light');}});function changeMode(theme){var isDark=theme==='dark';document.getElementById('theme-toggle').innerHTML=isDark?'🌙':'🌞';var oldChromaTheme=isDark?'chroma':'chroma-dark';var newChromaTheme=isDark?'chroma-dark':'chroma';var els=[].slice.apply(document.getElementsByClassName(oldChromaTheme));for(var i=0;i<els.length;i++){els[i].className=newChromaTheme;}}
function setMode(theme){window.localStorage.setItem('theme',theme);};function langSwitcherOver(){document.getElementById("langs").setAttribute("style","display:block");}
